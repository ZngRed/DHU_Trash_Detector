...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.cKitchen.setFormat("厨余垃圾: %v")
...
# 创建背景减除器对象
roi = (30, 40, 300, 360)  # 感兴趣区域
history_frame = 200
varThreshold = 150
detectShadows = False
learning_rate_threshold = 120
pixel_count_range = (500, 20000)
stable_frame_threshold = 25
report_interval = 8  # 以秒为单位
backSub = cv2.createBackgroundSubtractorMOG2(history=history_frame, varThreshold=varThreshold, detectShadows=detectShadows)
frame_count = 0
stable_frames = 0
last_report_time = 0
white_pixel_counts = []
...
def process_video(img, roi, learning_rate_threshold, pixel_count_range, stable_frame_threshold, report_interval):
    global frame_count, stable_frames, last_report_time, white_pixel_counts, report_num
    frame_count += 1
    learning_rate = 0.05 if frame_count <= learning_rate_threshold else 0
    ...
    # 应用背景减除算法于感兴趣区域
    fgMask_roi = backSub.apply(roi_frame, learningRate=learning_rate)
    
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))
    fgMask_roi = cv2.morphologyEx(fgMask_roi, cv2.MORPH_OPEN, kernel)

    # 统计感兴趣区域内的白色像素数量
    white_pixel_count = np.sum(fgMask_roi == 255)

    # 判断连续帧的白色像素数量是否在指定范围之间
    if pixel_count_range[0] <= white_pixel_count <= pixel_count_range[1]:
        stable_frames += 1
        if stable_frames >= stable_frame_threshold:
            current_time = time.time()
            if current_time - last_report_time >= report_interval:
                report_num += 1
                print("Item Exist!", "Frame:", frame_count, "White Pixel Count:", white_pixel_count, "report_num: ", report_num)
    ...
    # cv2.imshow('Foreground Mask in ROI', fgMask_roi)
    ...
    for path, im, im0s, vid_cap, s in dataset: # 每一帧
        ...
        for i, det in enumerate(pred):
            if webcam:  # batch_size >= 1
                ...
                process_video(im0, roi, learning_rate_threshold, pixel_count_range, stable_frame_threshold, report_interval)
                x, y, w, h = roi
                roi_frame = im0[y:y+h, x:x+w]
                # im0 = roi_frame
                s += f'{i}: '
            else:
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    print(c)
                    cls = 0
                    if c == 1 or c == 5 or c == 8 or c == 10:
                        cls = 0
                    if c == 11:
                        cls = 1
                    if c == 2 or c == 3 or c == 6:
                        cls = 2
                    if c == 4 or c == 7 or c == 9 or c == 12 or c == 0:
                        cls = 3
                    if cls == 0:
                        Tr = n.item()
                    if cls == 1:
                ...
                for *xyxy, conf, cls in reversed(det):
                    if view_img:  # Add bbox to image
                        ...
                        annotator.box_label(xyxy, label+str(c), color=colors(c, True))
            # 未识别到目标
            else:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'models/Trash_1201.pt', help='model path or triton URL')
    parser.add_argument('--source', type=str, default='0', help='file/dir/URL/glob/screen/0(webcam)')
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=(300,360), help='inference size h,w')
